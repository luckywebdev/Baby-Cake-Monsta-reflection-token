...
contract BABY_CAKE_MONSTA is ERC20, Ownable {
    ...
    BABYCAKEMONSTADividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public CAKE = address(0xF9f93cF501BFaDB6494589Cb4b4C15dE49E85D0e); //CAKE 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82
    address public MONSTA = address(0xC3F33A44ec2Ae141794a2327a804B487d29Dc684); //MONSTA //0x8A5d7FCD4c90421d21d30fCC4435948aC3618B2f

    uint256 public swapTokensAtAmount = 2000000 * (10**18);
    
    mapping(address => bool) public _isBlacklisted;

    uint256 public CAKERewardsFee = 50;
    uint256 public MONSTARewardsFee = 50;
    uint256 public liquidityFee = 25;
    uint256 public marketingFee = 25;
    uint256 public totalFees = CAKERewardsFee.add(MONSTARewardsFee).add(liquidityFee).add(marketingFee);

    address public _marketingWalletAddress = 0xC71756C0a393DA11ffeCe4A79980e7FB7952Dba4;
    ...
    constructor() public ERC20("BABY CAKE MONSTA", "BABYMONSTA") {

    	dividendTracker = new BABYCAKEMONSTADividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        BABYCAKEMONSTADividendTracker newDividendTracker = BABYCAKEMONSTADividendTracker(payable(newAddress));
    ...
    function setCAKERewardsFee(uint256 value) external onlyOwner{
        CAKERewardsFee = value.mul(10);
        totalFees = CAKERewardsFee.add(MONSTARewardsFee).add(liquidityFee).add(marketingFee);
    }

    function setLiquiditFee(uint256 value) external onlyOwner{
        liquidityFee = value.mul(10);
        totalFees = CAKERewardsFee.add(MONSTARewardsFee).add(liquidityFee).add(marketingFee);
    }

    function setMarketingFee(uint256 value) external onlyOwner{
        marketingFee = value.mul(10);
        totalFees = CAKERewardsFee.add(MONSTARewardsFee).add(liquidityFee).add(marketingFee);
    }

    function setMONSTARewardsFee(uint256 value) external onlyOwner{
        MONSTARewardsFee = value.mul(10);
        totalFees = CAKERewardsFee.add(MONSTARewardsFee).add(liquidityFee).add(marketingFee);
    }
    ...
    function swapAndSendDividends(uint256 tokens) private{
        uint256 tokenForCake = tokens.mul(CAKERewardsFee).div(CAKERewardsFee.add(MONSTARewardsFee));
        uint256 tokenForMonsta = tokens.sub(tokenForCake);
        swapTokensForCake(tokenForCake);
        swapTokensForMonsta(tokenForMonsta);
        uint256 dividendsForCake = IERC20(CAKE).balanceOf(address(this));
        bool successForCake = IERC20(CAKE).transfer(address(dividendTracker), dividendsForCake);
        uint256 dividendsForMonsta = IERC20(MONSTA).balanceOf(address(this));
        bool successForMonsta = IERC20(MONSTA).transfer(address(dividendTracker), dividendsForMonsta);

        if (successForCake && successForMonsta) {
            dividendTracker.distributeCAKEDividends(dividendsForCake.add(dividendsForMonsta));
            emit SendDividends(tokens, dividendsForCake.add(dividendsForMonsta));
        }
    }
}

contract BABYCAKEMONSTADividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    event Claim(address indexed account, uint256 amount, string withdrawType, bool indexed automatic);
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "BABYCAKE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
    ...
    function processAccount(address payable account, bool automatic) public onlyOwner returns (bool) {
    	...
    	if(amount > 0) {
            ...
            string memory withdrawType = 'CAKE';
            if(!_checkCakeEmit(account)) {
                withdrawType = 'MONSTA';
            }
            emit Claim(account, amount, withdrawType, automatic);
            _updateCakeEmit(account);
    		return true;
    	}
